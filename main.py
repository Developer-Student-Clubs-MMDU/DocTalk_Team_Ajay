...
model = genai.GenerativeModel('gemini-1.5-pro')  # Changed to pro version for image analysis

# Function to generate a response from the model
def get_gemini_response(user_input, conversation_history, image=None):
...
st.write("""
    ##  A GEN AI Chatbot for Medical Diagnosis and Recommendations
...
# Image upload option
uploaded_file = st.file_uploader("Upload an image related to your symptoms (optional)", type=["jpg", "jpeg", "png"])
image = None
...
if st.button('Continue'):
    if user_input or image:
        ...
        if st.session_state.conversation_stage == 'initial':
            prompt = f"""You are a medical chatbot. The user has described their initial symptoms or uploaded an image. 
            Ask 2-3 follow-up questions to gather more information. Focus on key symptoms, 
            duration, severity, and any relevant medical history. If an image is provided, analyze it for visual symptoms.
            ...
            if len(st.session_state.history) < 3:  # Continue asking follow-up questions
                ...
                follow-up questions to clarify the user's condition. If an image was provided, include questions about it.
                Do not provide a diagnosis yet.
                ...
                various medical fields. Based on the conversation history, symptoms described, and any images provided, 
        ...
        if prompt:  # Only call get_gemini_response if prompt is not empty
            response_text = get_gemini_response(prompt, conversation_history, image)
        ...
        st.error("Please enter some symptoms or upload an image to continue.")
